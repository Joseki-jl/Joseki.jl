...
### Until https://github.com/JuliaWeb/HTTP.jl/pull/427 is merged, we need to add multipart form
### support here.  This is based on https://github.com/JuliaWeb/HTTP.jl/blob/461a51fa247494df8aff116c418d0c324a6e14f0/src/parsemultipart.jl
...
function chunk2Multipart(chunk)
    ...
    return Multipart(string(filename), IOBuffer(content), string(contenttype))
end
...
"""
function parse_multipart_form(req::HTTP.Request)::Vector{Multipart}
    ...
    parse_multipart_body(req.body, boundaryDelimiter)
end
